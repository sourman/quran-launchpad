import { Button } from "@/components/ui/button";
import { BookOpen, Video, Calendar } from "lucide-react";

interface HeroProps {
  institutionName: string;
  tagline: string;
}

export const Hero = ({ institutionName, tagline }: HeroProps) => {
  return (
    <section className="relative min-h-screen flex items-center justify-center gradient-subtle overflow-hidden">
      <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-40"></div>
      
      <div className="container mx-auto px-4 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          <div className="mb-8 flex justify-center">
            <div className="p-4 rounded-full bg-primary/10 backdrop-blur-sm">
              <BookOpen className="w-16 h-16 text-primary" />
            </div>
          </div>
          
          <h1 className="text-5xl md:text-7xl font-bold mb-6 bg-clip-text text-transparent bg-gradient-to-r from-primary to-primary-light">
            {institutionName}
          </h1>
          
          <p className="text-xl md:text-2xl text-muted-foreground mb-12 max-w-2xl mx-auto">
            {tagline}
          </p>
          
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-16">
            <Button size="lg" className="gradient-primary shadow-elegant hover:shadow-glow transition-all duration-300">
              Start Learning Today
            </Button>
            <Button size="lg" variant="outline" className="border-2">
              Book a Free Trial
            </Button>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-3xl mx-auto">
            <div className="p-6 rounded-lg bg-card/50 backdrop-blur-sm border shadow-sm hover:shadow-md transition-all">
              <Video className="w-10 h-10 text-primary mb-4 mx-auto" />
              <h3 className="font-semibold mb-2">Live Online Classes</h3>
              <p className="text-sm text-muted-foreground">Interactive video sessions with expert teachers</p>
            </div>
            
            <div className="p-6 rounded-lg bg-card/50 backdrop-blur-sm border shadow-sm hover:shadow-md transition-all">
              <Calendar className="w-10 h-10 text-accent mb-4 mx-auto" />
              <h3 className="font-semibold mb-2">Flexible Scheduling</h3>
              <p className="text-sm text-muted-foreground">Book classes at times that work for you</p>
            </div>
            
            <div className="p-6 rounded-lg bg-card/50 backdrop-blur-sm border shadow-sm hover:shadow-md transition-all">
              <BookOpen className="w-10 h-10 text-primary mb-4 mx-auto" />
              <h3 className="font-semibold mb-2">Structured Curriculum</h3>
              <p className="text-sm text-muted-foreground">Comprehensive Quran learning programs</p>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};